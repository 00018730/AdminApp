...
export default function PaymentsSection() {
  ...
  const [payAmount, setPayAmount]   = useState('550000')
  ...
  const save = async () => {
    ...
    await supabase.from('payments').insert({ student_username:payStudent, month, payment_month:month, amount:parseInt(payAmount), status:'paid', method:payMethod, payment_date:payDate, cheque_number:num, notes:payNotes||null })
    await fetchAll()
    setShowForm(false); setPayStudent(''); setPayTeacher(''); setPayGroup(''); setPayNotes('')
    setSaving(false)
  }
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'22px', flexWrap:'wrap', gap:'10px' }}>
        ...
        <button onClick={() => setShowForm(true)} style={{ padding:'9px 18px', borderRadius:'8px', border:'none', background:G, color:'white', fontSize:'13px', fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:'700', cursor:'pointer' }}>+ Record payment</button>
      </div>
        ...
        <div style={{ background:'white', borderRadius:'12px', border:'1.5px solid #fca5a5', padding:'20px', marginBottom:'20px' }}>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'14px' }}>
            ...
            <div style={{ display:'flex', gap:'6px' }}>
              ...
              <button onClick={() => { setShowUnpaid(false); setUnpaidTeacher(null) }} style={{ padding:'5px 12px', borderRadius:'6px', border:'1.5px solid #e4e8e7', background:'white', color:'#64748b', fontSize:'12px', fontWeight:'600', cursor:'pointer' }}>✕ Close</button>
            </div>
          </div>
          ...
          {!unpaidTeacher ? (
            <div style={{ display:'flex', flexDirection:'column', gap:'6px' }}>
              ...
              {teachers.every(t => !students.filter(s => s.teacher_username === t.username && !paidSet.has(s.username)).length) && (
                <div style={{ textAlign:'center', padding:'20px', color:'#94a3b8', fontSize:'13px' }}>🎉 All students paid for this month!</div>
              )}
            </div>
          ) : (
                ...
                {students.filter(s => s.teacher_username === unpaidTeacher.username && !paidSet.has(s.username)).map((s,i) => (
                  <tr key={i} style={{ borderBottom:'1px solid #f0f2f1' }}>
                    ...
                    <td style={{ padding:'10px 12px' }}>
                      <button onClick={() => {
                        setPayStudent(s.username); setPayTeacher(s.teacher_username)
                        const [d,t] = (s.day+'-'+s.class_time).split(/-(?=\d)/)
                        setPayGroup(s.day+'-'+s.class_time)
                        setShowUnpaid(false); setUnpaidTeacher(null); setShowForm(true)
          ...
          <tbody>
            {listed.map((p,i) => (
              <tr key={i} style={{ borderBottom: i<listed.length-1 ? '1px solid #f0f2f1' : 'none' }}
                ...
                <td style={{ padding:'12px 16px', fontFamily:'monospace', fontSize:'12px', color:G, fontWeight:'600' }}>{p.cheque_number||'—'}</td>
                ...
                <td style={{ padding:'12px 16px', color:'#64748b', fontSize:'13px' }}>{p.payment_date||'—'}</td>
                <td style={{ padding:'12px 16px' }}>
                  {p.cheque_number && (
                    <button onClick={() => downloadCheque(p, p.students?.full_name||p.student_username||'')}
                      style={{ padding:'5px 12px', borderRadius:'6px', border:`1.5px solid ${G}`, background:`${G}10`, color:G, fontSize:'12px', fontWeight:'600', cursor:'pointer' }}>
                      🧾 Receipt
                    </button>
                  )}
                </td>
            ...
            {!listed.length && (
              <tr><td colSpan="6" style={{ padding:'40px', textAlign:'center', color:'#94a3b8' }}>No payments recorded for {monthOpts.find(o=>o.val===month)?.label}</td></tr>
            )}
          </tbody>
        </table>
      </div>
      ...
      {showForm && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.45)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:200 }}
          ...
          <div style={{ background:'white', borderRadius:'16px', padding:'28px', width:'460px', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 24px 64px rgba(0,0,0,0.18)' }}>
            <h3 style={{ fontSize:'18px', fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:'800', color:D, marginBottom:'20px' }}>Record Payment</h3>
            ...
            <div style={{ display:'flex', gap:'10px', marginTop:'8px' }}>
              <button onClick={() => { setShowForm(false); setPayStudent(''); setPayTeacher(''); setPayGroup(''); setPayNotes('') }}
              ...
              <button onClick={save} disabled={saving||!payStudent}
                style={{ flex:1, padding:'12px', borderRadius:'8px', border:'none', background:G, color:'white', fontSize:'14px', fontFamily:"'Plus Jakarta Sans',sans-serif", fontWeight:'700', cursor:'pointer', opacity:!payStudent?.6:1 }}>
